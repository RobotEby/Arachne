        ...
        if os.access(docker_socket, os.R_OK):
            ...
            try:
                ...
                if output.returncode == 0:
                    ...
                    results["details"] = (
                        f"Docker API accessible: {docker_info.get('Version')}"
                    )
...
def check_for_breakout_techniques():
    ...
    try:
        ...
        if any(
            vuln in kernel_version for vuln in ["3.13", "3.16", "3.19", "4.4", "4.8"]
        ):
        ...
        docker_version = subprocess.getoutput(
            "docker version 2>/dev/null | grep Version | head -1"
        )
