        ...
        try:
            for root, dirs, files in os.walk(self.hidden_dir):
                for file in files:
                    ...
                    subprocess.call(
                        f'setfattr -n user.hidden -v 1 "{filepath}" 2>/dev/null',
                        shell=True,
                    )
        ...
        try:
            for root, dirs, files in os.walk(self.hidden_dir):
                for file in files:
                    ...
                    subprocess.call(
                        f'setfacl -m u:nobody:--- "{filepath}" 2>/dev/null', shell=True
                    )
                    subprocess.call(
                        f'setfacl -m g:nogroup:--- "{filepath}" 2>/dev/null', shell=True
                    )
