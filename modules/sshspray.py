...
class SSHSprayer:
    def __init__(
        ...
        threads=5,
        timeout=5,
        output_dir="~/.cache/.arachne/ssh_spray",
    ):
    ...
    def execute(self):
        ...
        try:
            ...
            print(
                f"[+] Total attempts: {len(self.targets) * len(self.usernames) * len(self.passwords)}"
            )
            ...
            report = {
                ...
                "sample_errors": self.results["errors"][:10]
            ...
            output_file = os.path.join(
                self.output_dir,
                f"ssh_spray_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            )
            ...
            creds_file = os.path.join(
                self.output_dir,
                f"ssh_credentials_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
            )
            ...
            print(
                f"[+] SSH spray complete. Found {len(self.results['successful'])} valid credentials."
            )
...
def arachne_integration():
    ...
    parser = argparse.ArgumentParser(description="Arachne SSH Spray")
    parser.add_argument(
        "--targets", help="Target IPs or ranges (comma-separated or file)"
    )
    ...
    if args.username_file or args.password_file:
        sprayer.load_credentials_from_file(
            args.username_file or "", args.password_file or ""
        )
