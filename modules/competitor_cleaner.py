        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline", "username"]):
            try:
                ...
                for pattern in self.suspicious_patterns["process_names"]:
                    if re.search(pattern, pinfo["name"], re.I) or re.search(
                        pattern, cmdline, re.I
                    ):
                        suspicious.append(
                            {
                                "pid": pinfo["pid"],
                ...
                for root, dirs, files in os.walk(scan_path):
                    for file in files:
                        ...
                        if any(filepath.endswith(ext) for ext in suspicious_exts):
                            suspicious_files.append(
                                {"path": filepath, "reason": "Suspicious extension"}
                            )
                ...
                if cron["source"] == "user_crontab":
                    ...
                    if len(new_lines) != len(lines):
                        ...
                        if new_lines:
                            crontab_content = "\n".join(new_lines)
                            subprocess.call(
                                f'echo "{crontab_content}" | crontab -', shell=True
                            )
        ...
        try:
            ...
            if processes or files or crons:
                ...
                report_file = os.path.expanduser(
                    "~/.cache/.arachne/competitor_cleanup.json"
                )
