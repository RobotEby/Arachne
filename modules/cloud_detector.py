...
class CloudDetector:
    ...
    def detect_aws(self):
        ...
        try:
            ...
            if response.getcode() == 200:
                ...
                print(
                    f"       Instance Type: {self.cloud_metadata.get('aws_instance-type', 'Unknown')}"
                )
                print(
                    f"       Region: {self.cloud_metadata.get('aws_region', 'Unknown')}"
                )
                return True
    ...
    def detect_azure(self):
        ...
        try:
            ...
            if response.getcode() == 200:
                ...
                if "compute" in data:
                    ...
                    self.cloud_metadata["azure_subscriptionId"] = compute.get(
                        "subscriptionId"
                    )
                    self.cloud_metadata["azure_resourceGroupName"] = compute.get(
                        "resourceGroupName"
                    )

                print(f"[CLOUD] Detected Azure VM")
                print(
                    f"       VM Size: {self.cloud_metadata.get('azure_vmSize', 'Unknown')}"
                )
                print(
                    f"       Location: {self.cloud_metadata.get('azure_location', 'Unknown')}"
                )
                return True
        ...
        azure_indicators = [
            (
                ...
                lambda x: "7783-7084-3265-9085-8269-3286-77" in x,
            ),
            ("/sys/class/dmi/id/sys_vendor", lambda x: "microsoft" in x.lower()),
            (
                "/sys/class/dmi/id/product_name",
                lambda x: "virtual machine" in x.lower() and "microsoft" in x.lower(),
            ),
    ...
    def detect_gcp(self):
        ...
        try:
            ...
            if response.getcode() == 200:
                ...
                print(
                    f"       Project: {self.cloud_metadata.get('gcp_project_id', 'Unknown')}"
                )
        ...
        gcp_indicators = [
            ...
            (
                ...
                lambda x: "gcp" in x.lower() or "google" in x.lower(),
            ),
        ]
    ...
    def detect_container(self):
        ...
        container_indicators = [
            ...
            (
                "/proc/1/sched",
                lambda x: "bash" not in x and "init" not in x,
            ),  # Non-standard init process
        ]
    ...
    def detect_vm(self):
        ...
        vm_indicators = [
            ...
            (
                ...
                lambda x: any(
                    vendor in x.lower()
                    for vendor in ["vmware", "innotek", "qemu", "microsoft", "xen"]
                ),
            ),
