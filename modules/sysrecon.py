...
class SystemRecon:
    def __init__(self, output_dir="~/.cache/.arachne/recon"):
    ...
    def get_last_login(self, username):
        ...
        try:
            output = subprocess.check_output(
                f"last -n 5 {username}", shell=True, stderr=subprocess.DEVNULL
            ).decode()
    ...
    def get_network_info(self):
        ...
        try:
            ...
            for iface, stats in routes.items():
                network["routing"].append(
                    {"interface": iface, "stats": stats._asdict()}
                )
        except:
            pass
    ...
    def get_hardware_info(self):
        ...
        try:
            ...
            hardware["cpu"] = {
                ...
                if re.search(r"model name", cpu_info)
                else "Unknown",
            }
        ...
        try:
            ...
            if os.path.exists("/etc/debian_version"):
                packages = subprocess.check_output(
                    "dpkg -l | grep ^ii", shell=True, stderr=subprocess.DEVNULL
                ).decode()
                software["packages"] = packages.split("\n")[:50]
            elif os.path.exists("/etc/redhat-release"):
                packages = subprocess.check_output(
                    "rpm -qa", shell=True, stderr=subprocess.DEVNULL
                ).decode()
                software["packages"] = packages.split("\n")[:50]
            ...
            services = []
            for service in (
                psutil.win_service_iter() if platform.system() == "Windows" else []
            ):
            ...
            cron_output = subprocess.check_output(
                "crontab -l 2>/dev/null || echo 'No cron for user'", shell=True
            ).decode()
    ...
    def get_defense_status(self):
        ...
        try:
            ...
            if os.path.exists("/usr/sbin/sestatus"):
                selinux = subprocess.check_output(
                    "sestatus", shell=True, stderr=subprocess.DEVNULL
                ).decode()
            ...
            try:
                firewall = subprocess.check_output(
                    "iptables -L -n 2>/dev/null || ufw status 2>/dev/null || echo 'No firewall'",
                    shell=True,
                ).decode()
                defenses["firewall"] = (
                    "Chain INPUT" in firewall or "active" in firewall.lower()
                )
            except:
                pass
    ...
    def execute(self):
        ...
        try:
            ...
            output_file = os.path.join(
                self.output_dir,
                f"sysrecon_{socket.gethostname()}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            )
